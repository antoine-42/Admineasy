function Mamachine(lien_, ip_)
	{
		this.lien = lien_;
		this.ip = ip_;
		this.etat="connected";
		console.log("MAAAAAAAAAACHIIIIIIIIIIINE");
		...
		this.verifier_connexion = function()
									{
										console.log("VERIFIER_CONNEXION")
										var requete= creerRequete() ;
										var url="http://nailyk.ddns.net:54824/ping?ip="+this.ip ;
										...
										if(requete.readyState==4 && requete.status==200)
										{
											...
											//Vérifie dans la chaine de retour du serveur la présence de la condition
											if(/deconnecté/.test(maReponse)) //inverser le test pour la production --> gain de performance /!\ mettre !(recherche)
												{
													this.etat="disconnected" ;
												}
											console.log("ETAAAAAAAAT = "+this.etat);
										}
										this.affichage();
									}
...
var Methode =
{
    ...
    parcourirResultat: function(requete){  // est appelé par searchListReseau
		...
		for (var i = 0 ; i < maReponse.length; i++){  
			// parcours la reponse envoyé par le serveur
				...
				if(maReponse.charAt(i)=='<'){
					...
					if(maReponse.charAt(i)=='/'){
						...
						if(maReponse.charAt(i)=='a'){
							i++;
							if(maReponse.charAt(i)=='>'){ // dès qu'une fin de balise a est detectée
								console.log("AFFICHER IMAGE ");
								var s=maReponse.substring(j, i);
								var ip=Methode.isolerIp(s);
								var m = new Mamachine(s, ip);
								// affiche la partie de la reponse du serveur
								// qui correspond à un lien 
								j=i+1;  // pour decaler la partie que l'on va afficher
								console.log("TEEEEEEEEEEEEEEEEEEEEEEEEEESSSSSSSSSSSSSSST")
	...
	searchListMachine : function(){  // affiche liste machines avec les caracterestique principales
